    ...
    (<div
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-10 ">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
            <div
              ...
              <h3
                className="hidden md:block text-lg md:pl-20 md:text-3xl font-bold text-neutral-500 ">
                {item.title}
              </h3>
            </div>
        ...
        <div
          ...
          className="absolute md:left-8 left-8 top-0 overflow-hidden w-[7px] bg-[linear-gradient(to_bottom,var(--tw-gradient-stops))] from-transparent from-[0%] via-neutral-200 dak:via-neutral-700 to-transparent to-[99%]  [mask-image:linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)] ">
          <motion.div
            style={{
              height: heightTransform,
              opacity: opacityTransform,
            }}
            className="absolute inset-x-0 top-0  w-[7px] bg-gradient-to-t from-purple-500 via-orange-500 to-transparent from-[0%] via-[10%] rounded-full" />
        </div>
      </div>
    </div>)
  );
